...
if [ ! -d "$ROOTDIR/libs/garden/garden.zbarcam" ];then
    ...
    cp $ROOTDIR/libs/garden/garden.zbarcam/opencv-4.0.1/build/lib/cv2.so $VENVDIR/lib/python3.$MINOR_V/site-packages/
    cp $ROOTDIR/libs/garden/garden.zbarcam/opencv-4.0.1/build/lib/python3/cv2*.so $VENVDIR/lib/python3.$MINOR_V/site-packages/
    cd "$ROOTDIR"
fi
